...
public final class Patterns {
  ...
  /**
   * Modifies given {@code pattern} to empty string {@link Pattern} if it can match empty string.
   *
   * <p><em>nullable</em> {@link Pattern} is a simplification of an input {@link Pattern} that may be convenient when
   * using derivatives. If the {@code pattern} matches the empty string, then the nullable pattern will match the empty
   * string, otherwise it will match {@link #NEVER}.</p>
   *
   * @param  pattern nested pattern to make <em>nullable</em>.
   *
   * @return a {@link Pattern} that's a nullable version of given {@code pattern}.
   */
  public static Pattern nullable(final Pattern pattern) {
    return new NullablePattern(pattern);
  }
  ...
  /** Returns a {@link Pattern} object that matches if the input has {@code n} occurrences of {@code pattern}. */
  static Pattern repeat(final int n, final Pattern pattern) {
    Checks.checkNonNegative(n, "n < 0");
    return new RepeatPattern(n, pattern);
  }
  ...
  static Pattern many(final int min, final Pattern pattern) {
    Checks.checkMin(min);
    return new ManyBoundedPattern(min, pattern);
  }

  static Pattern many(final Pattern pattern) {
    return new ManyPattern(pattern);
  }
  ...
  static Pattern some(final int max, final Pattern pp) {
    Checks.checkMax(max);
    return new SomePattern(max, pp);
  }

  /**
   * Returns a {@link Pattern} that tries both {@code p1} and {@code p2}, and picks the one with the longer match
  ...
  /**
   * Returns a {@link Pattern} that tries all of {@code patterns}, and picks the one with the longest match length. If
